"""
Training script — UNetFormer baseline: encoder ResNet-18 (pretrained),
decoder GLTB (Global-Local Transformer Block, paper-faithful), loss =
CombinedLoss (CrossEntropy + Dice) trên Fused Feature của decoder.

Đây là baseline (L_region thuần, không boundary/affinity loss) cho đề tài
"Dynamic Boundary-Aware Loss via Adaptive BCE-Affinity Coupling"
(idead_research.md) — tương ứng hàng "Baseline (L_region)" trong Bảng 1 của
đề cương.

File này ĐỘC LẬP với các script train_unet_former_reshnet101_32*.py (encoder
ResNeXt101_32x16d) và train_cnn*.py hiện có trong project — chỉ dùng chung
src/data, src/utils (dataset/transforms/losses/metrics/callbacks/visualizer
không đổi), model riêng (unet_former_resnet18.py), để không ảnh hưởng các
thí nghiệm cũ.

Khác biệt so với các script train_*_combineLoss.py cũ:
    - Model: unet_former_resnet18.build_model (encoder resnet18, ~11.6M tham
      số — nhẹ hơn ResNeXt101_32x16d khoảng 16-17 lần) thay vì
      unet_former_reshnet101_32_combineLoss.
    - BASE_LR mặc định trong config nên đặt ở mức cao hơn nhiều so với
      1e-5 dùng cho ResNeXt101_32x16d(SWSL) — vì backbone ở đây nhẹ, chỉ
      pretrain ImageNet/SWSL thông thường, không cần LR cực nhỏ để tránh phá
      vỡ đặc trưng đã học từ tập ảnh khổng lồ 1 tỷ ảnh như ResNeXt101_32x16d.
      Paper UNetFormer gốc dùng base_lr=6e-4 (backbone_lr=6e-5, layerwise) —
      khuyến nghị base_lr=6e-4 trong config, khớp với Track A (ResNet-101,
      CNN decoder) đã dùng trong project này.

Usage (Kaggle, 2x T4):
    torchrun --nproc_per_node=2 src/train_unet_former_resnet18.py --config configs/unet_former_resnet18_combineLoss/luot1_500.yaml
    torchrun --nproc_per_node=2 src/train_unet_former_resnet18.py --config configs/unet_former_resnet18_combineLoss/luot1_500.yaml --dry-run
    torchrun --nproc_per_node=2 src/train_unet_former_resnet18.py --config configs/unet_former_resnet18_combineLoss/luot1_500.yaml --resume <work_dir>/latest_checkpoint.pth
"""
...
# Kaggle can mount the same dataset under different paths across sessions, and
# different mirrors of OpenEarthMap name the label folder "labels/" (plural,
# the Kaggle mirror `aletbm/global-land-cover-mapping-openearthmap`) or
# "label/" (singular, some local/alternate copies). Detect this at runtime
# instead of trusting the YAML blindly, so the same config works across
# environments.

def find_data_base(data_root: str) -> str:
    """Auto-detect the actual dataset base by probing common subdirectory layouts."""
    candidates = [
        data_root,
    ...
    for c in candidates:
        if os.path.isdir(os.path.join(c, 'images', 'train')) or \
           os.path.isdir(os.path.join(c, 'images', 'val')):
            return c
    return data_root  # fall through so downstream raises a clear FileNotFoundError
...
def find_label_subdir(base: str) -> str:
    ...
    for name in ('labels', 'label'):
        if os.path.isdir(os.path.join(base, name)):
            return name
    return 'labels'  # default, matches the official Kaggle mirror


def resolve_dataset_paths(ds: dict) -> None:
    """Mutates `ds` (the DATASET section of the config) in place: resolves
    ROOT_DIR/VAL_ROOT_DIR to their real mounted location, and fixes
    TRAIN_MASK_DIR/VAL_MASK_DIR if the YAML says "labels/..." but the real
    folder is "label/..." (or vice-versa)."""
...
def set_seed(seed: int):
    """Đặt seed chung cho random/numpy/torch để các lượt thí nghiệm đều khởi
    tạo model (decoder/GLTB random init) và shuffle dữ liệu theo đúng cùng 1
    seed, chỉ khác nhau ở biến đang khảo sát (lượng dữ liệu train / loss
    variant / alpha / fold)."""
    random.seed(seed)
    np.random.seed(seed)
...
def setup_log_file(path: str):
    """Open WORK_DIR/train_log.txt in append mode so log() persists every run
    to disk (survives losing the Kaggle notebook output) and keeps prior runs'
    history on resume instead of overwriting it."""
    global _log_file
    if not is_main():
        return
...
def main():
    ...
    parser.add_argument('--data-root', default=None,
                        help='Override DATASET.ROOT_DIR/VAL_ROOT_DIR from the config '
                             '(e.g. if Kaggle mounted the dataset at a different path this '
                             'session) — no split files involved, just where images/train and '
                             'images/val are read from directly.')
    args = parser.parse_args()

    # ── DDP init ────────────────────────────────────────────────────────────
    # Use nccl+CUDA on Kaggle (unchanged behaviour); fall back to gloo+CPU
    # automatically when no GPU is present (local dry-run/dev only).
    ...
    # Auto-resume: if --resume wasn't passed but a checkpoint already exists in
    # WORK_DIR (e.g. re-running the same launch command after a Kaggle session
    # was interrupted), pick it up automatically instead of retraining from
    # scratch.
    ...
    # Resolve dataset root(s)/label-subdir dynamically — Kaggle mount paths and
    # label folder naming ("labels/" vs "label/") can differ across environments.
    resolve_dataset_paths(ds)

    # ── Datasets ─────────────────────────────────────────────────────────────
    # Baseline: NO split file at all — every image already under TRAIN_IMG_DIR /
    # VAL_IMG_DIR is used as-is (the dataset's own existing train/val
    # partition). TRAIN_SPLIT_FILE/VAL_SPLIT_FILE are optional and only kept
    # for future use (e.g. a 5-fold spatial CV list) — omit them in the config
    # to use the whole directory directly.
    ...
    if not train_split and not val_split:
        log(f"Dataset: {len(train_ds)} train images (no split file — full {ds['TRAIN_IMG_DIR']}), "
            f"{len(val_ds)} val images (no split file — full {ds['VAL_IMG_DIR']})")
    else:
        log(f"Dataset: {len(train_ds)} train images, {len(val_ds)} val images "
            f"(split file: train={train_split}, val={val_split})")
    ...
    # CombinedLoss = CrossEntropy + Dice, tính trên logits suy ra từ Fused
    # Feature (output decoder.frh -> classifier) — đúng L_region của baseline.
    ...
    history         = []     # list of {iter, val_round, mIoU, val_loss, iou_<class>…}
    total_val_rounds = -(-tr['MAX_ITERS'] // tr['VAL_INTERVAL'])  # ceil division
    ...
    log(f"Training {tr['MAX_ITERS']} iterations | "
        f"encoder={mdl['ENCODER']} | loss=CombinedLoss(CE+Dice) | "
    ...
    for iteration in range(start_iter, tr['MAX_ITERS'] + 1):
        ...
        if iteration % tr['VAL_INTERVAL'] == 0 or iteration == tr['MAX_ITERS']:
            ...
            val_result = validate(
                ...
                device, amp_enabled=amp_enabled,
            )
            miou = val_result['mIoU']
            log(f"  [Val round {val_round}/{total_val_rounds}, iter {iteration}] "
                f"mIoU={miou:.4f}  val_loss={val_result['val_loss']:.4f}")
            ...
            if is_main():
                ...
                # Save visualisations (segmentation + boundary)
    ...
    if is_main():
        ...
        # final_summary.txt — best val round + per-class IoU + total run time,
        # in one human-readable file (benchmark_results.csv has the same data
        # machine-readable, one row per validation).
        _cls = ['Background', 'Bareland', 'Rangeland', 'Developed', 'Road',
                'Tree', 'Water', 'Agriculture', 'Building']
        summary_path = os.path.join(out['WORK_DIR'], 'final_summary.txt')
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write('=== UNetFormer (ResNet-18) baseline — final summary ===\n\n')
            ...
            if best_per_class is not None:
                for name, iou_v in zip(_cls, best_per_class):
