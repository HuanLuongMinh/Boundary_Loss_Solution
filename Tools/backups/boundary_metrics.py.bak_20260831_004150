"""BoundaryMetrics — Boundary IoU(d) / BF-Score / ASD cho "Bước 6" của
docs/workflow_2.md mục 4 (TODO chốt với Huan 29/8/2026). File MỚI, độc lập —
không sửa src/utils/metrics.py hiện có, chỉ theo ĐÚNG pattern của
SegmentationMetrics trong file đó: update() tích luỹ theo batch (KHÔNG average
theo batch — cộng dồn pixel-count qua toàn bộ dataset rồi mới chia ở
compute(), giống hệt cách SegmentationMetrics dồn confusion matrix),
compute() trả dict.

KHÔNG CẦN GPU để TÍNH các chỉ số này — toàn bộ dùng numpy/scipy trên CPU
(distance_transform_edt). GPU chỉ cần cho bước forward pass ra `logits` (đã có
sẵn trong vòng lặp train/eval hiện tại) — BoundaryMetrics chỉ nhận
logits/targets đã có sẵn (có thể trên GPU hoặc CPU, tự chuyển .cpu().numpy()
bên trong), không tự chạy model. Retro-fit chỉ số này cho checkpoint Run 1/
Run 2 cũ (xem Tools/eval_boundary_metrics.py) chỉ cần 1 lượt forward pass (có
GPU thì nhanh hơn, không có vẫn chạy được vì ResNet-18 nhẹ).

3 nhóm chỉ số, theo đúng spec docs/workflow_2.md mục 4:

1. Boundary IoU(d) — Cheng et al., "Boundary IoU: Improving Object-Centric
   Image Segmentation Evaluation", CVPR 2021. Định nghĩa THEO TỪNG LỚP
   (per-class, giống per_class_iou trong SegmentationMetrics): với 1 mask nhị
   phân M của lớp c, "dải biên" (boundary band) là tập pixel thuộc M và cách
   biên ngoài của M không quá d pixel (đo bằng distance_transform_edt tính BÊN
   TRONG mask — khoảng cách tới pixel nền/không-thuộc-M gần nhất). Boundary
   IoU(d) = IoU thông thường nhưng CHỈ tính trên phần giao 2 dải biên (GT và
   pred) — phạt nặng lỗi định vị biên, bỏ qua lỗi ở vùng lõi xa biên. Trung
   bình trên các lớp có mặt trong GT hoặc pred (union > 0 ở ít nhất 1 sample),
   giống valid_classes trong SegmentationMetrics.compute().

2. BF-Score (Boundary F1) — precision/recall giữa tập pixel "biên" của GT và
   của prediction trong ngưỡng khoảng cách `bf_tolerance` pixel. Biên ở đây
   dùng ĐÚNG 1 ĐỊNH NGHĨA CHUNG với boundary_bce.py/affinity.py —
   `extract_edge_gt()`, biên = pixel có ít nhất 1 hàng-xóm 4-connected (hoặc
   8, tuỳ config) VALID mang nhãn khác, KHÔNG phải biên theo từng lớp riêng
   lẻ. Bắt buộc dùng lại đúng hàm này (không định nghĩa "biên" kiểu khác cho
   riêng metric) — đúng yêu cầu tường minh docs/workflow_2.md mục 3.1.

3. ASD (Average Surface Distance) — trung bình khoảng cách Euclidean 2 chiều
   (pred→GT và GT→pred) giữa 2 tập pixel biên (cùng định nghĩa biên như
   BF-Score), dùng scipy.ndimage.distance_transform_edt (KHÔNG vòng lặp
   pixel-by-pixel).

Pixel ignore (`mask == ignore_index`) bị loại khỏi CẢ GT lẫn prediction trước
khi tính, dùng validity mask lấy từ chính extract_edge_gt trả về — không tự
định nghĩa lại.

Lưu ý về giá trị d dùng trong báo cáo: spec tổng quát ở đây (và default của
class) là d ∈ {1,3,5} theo đúng Cheng et al. Tuy nhiên `summary.txt` do
src/train_bce_edge.py::write_bce_edge_summary() sinh ra (đã tồn tại, không
được sửa) in cố định 3 nhãn dòng "Best Boundary IoU @1/@2/@4" — tức là d=1,2,4
— nên Tools/eval_boundary_metrics.py (script điền số thật vào các dòng đó)
gọi BoundaryMetrics với boundary_distances=(1,2,4) để khớp đúng nhãn đã có sẵn
trong file đó, dù class này vẫn giữ default (1,3,5) cho mục đích tổng quát.

Tự-test: chạy trực tiếp `python -m src.utils.boundary_metrics` (từ repo root)
để verify các case biết trước đáp số (mask trùng khớp hoàn toàn -> BIoU/BF=1.0,
ASD=0; lệch 1 pixel có kiểm soát; toàn bộ ignore -> không NaN/crash; batch
không lớp nào xuất hiện -> compute() vẫn trả dict hợp lệ).
"""
...
class BoundaryMetrics:
    ...
    (numpy/scipy) — gọi update() ngay sau (hoặc cùng chỗ)
    SegmentationMetrics.update() hiện có trong validate(), không cần đổi vị
    trí gọi trong vòng lặp."""

    def __init__(self, num_classes: int, ignore_index: int = 255,
                 boundary_distances=(1, 3, 5), bf_tolerance: int = 2,
                 connectivity: int = 4, dilation_radius: int = 0):
        """
        num_classes/ignore_index: PHẢI khớp SegmentationMetrics đang dùng (9
            và 255 theo baseline hiện tại).
        boundary_distances: các giá trị d (pixel) để tính Boundary IoU(d) —
            mặc định (1,3,5) đúng docs/workflow_2.md mục 4 (Cheng et al.).
        bf_tolerance: ngưỡng khoảng cách (pixel) khi SO KHỚP 2 tập biên đã
            trích xuất cho BF-Score — 1 pixel biên dự đoán được tính "khớp"
            nếu có >=1 pixel biên GT trong bán kính này (và ngược lại cho
            recall). KHÔNG nhầm với `dilation_radius` của extract_edge_gt (đó
            là làm dày biên NGAY KHI ĐỊNH NGHĨA edge_gt, còn bf_tolerance là
            dung sai lúc so khớp 2 tập biên đã có sẵn — 2 khái niệm độc lập,
            có thể cùng >0).
        connectivity/dilation_radius: PHẢI khớp giá trị đã cố định cho toàn
            nghiên cứu trong boundary_bce.py (mặc định connectivity=4,
            dilation_radius=0, đúng configs/unet_former_resnet18_bce_edge/
            bce_edge.yaml hiện tại) — lệch giá trị này giữa các run sẽ phá
            tính so sánh được của Bảng 1.
        """
    ...
    def reset(self):
        # Boundary IoU: cộng dồn intersection/union theo pixel-count,
        # per-class, per-distance — qua TOÀN BỘ dataset (không average theo
        # batch/ảnh), giống hệt triết lý confusion matrix trong
        # SegmentationMetrics.
    ...
    def update(self, logits: torch.Tensor, targets: torch.Tensor):
        ...
        for b in range(preds_np.shape[0]):
            ...
            for c in range(self.num_classes):
                ...
                # Tính distance_transform_edt 1 LẦN mỗi mask (không lặp lại
                # cho từng d) rồi threshold nhiều mức d — tiết kiệm ~3x so với
                # gọi scipy riêng cho mỗi d.
        ...
        for b in range(gt_edge_np.shape[0]):
            ...
            # distance_transform_edt(~X): khoảng cách từ MỌI pixel tới pixel
            # biên gần nhất trong X — tra cứu 1 lần cho toàn ảnh, không lặp pixel.
            if n_gt > 0:
